...
double calculateCrossKinkStress(double a, double ksp, double angle, double kinkWidth)
{
	double A[3] = {h,0,-2000};
	double B[3] = {h,0,-kinkWidth*b*0.5};
	double C[3] = {0,0,+kinkWidth*b*0.5};
	double D[3] = {0,0,(ksp-kinkWidth*0.5)*b};
	double E[3] = {h*cos(angle),h*sin(angle),(ksp+0.5*kinkWidth)*b};
	double F[3] = {h*cos(angle),h*sin(angle),2000};

	double totalStress[6] = {0};
#if 0
	for(int s = 0; s < 1000; s++) {
	double probePoint[3] = { h * s / 1000, 0, 0 };
	...
	segmentStress(probePoint, D, E, b_spatial, totalStress, a);
	segmentStress(probePoint, E, F, b_spatial, totalStress, a);
	}
	return totalStress[4] /1000;
#else
	double probePoint[3] = { h * 1, 0, 0 };
	...
	segmentStress(probePoint, D, E, b_spatial, totalStress, a);
	segmentStress(probePoint, E, F, b_spatial, totalStress, a);
	return totalStress[4];
#endif
}

int main(int argc, char* argv[])
{
	ofstream stream0_sharp("cross_kink_0_stress_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream0_sharp << ksp << " " << calculateCrossKinkStress(0.5 * b, ksp, 0.0, 0) << endl;
	}

	ofstream stream60_sharp("cross_kink_60_stress_0.5b_1.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream60_sharp << ksp << " " << calculateCrossKinkStress(0.5 * b, ksp, M_PI/3.0, 0) << endl;
	}

	ofstream stream120_sharp("cross_kink_120_stress_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream120_sharp << ksp << " " << calculateCrossKinkStress(0.5 * b, ksp, M_PI*2.0/3.0, 0) << endl;
	}

	ofstream stream180_sharp("cross_kink_180_stress_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream180_sharp << ksp << " " << calculateCrossKinkStress(0.5 * b, ksp, M_PI, 0) << endl;
	}

	ofstream stream60_beveled("cross_kink_60_beveled_stress_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream60_beveled << ksp << " " << calculateCrossKinkStress(0.5 * b, ksp, M_PI/3.0, kinkWidth) << endl;
	}

	ofstream stream180_beveled("cross_kink_180_beveled_stress_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream180_beveled << ksp << " " << calculateCrossKinkStress(0.5 * b, ksp, M_PI, kinkWidth) << endl;
	}
