...
double calculateStressSharpKinkPair(double a, double ksp)
{
	double A[3] = {h,0,-2000};
	double B[3] = {h,0,0};
	double C[3] = {0,0,0};
	double D[3] = {0,0,ksp*b};
	double E[3] = {h,0,ksp*b};
	double F[3] = {h,0,2000};

	double probePoint[3] = { h/2, 0, 0 };
...
{
	ofstream stream_beveled("kink_pair_stress_beveled_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream_beveled << ksp << " " << calculateStressBeveledKinkPair(0.5 * b, ksp) << endl;
	}

	ofstream stream_sharp("kink_pair_stress_sharp_0.5b.dat");
	for(int i = 0; i < 1000; i++) {
		double ksp = (double)i / 50.0;
		stream_sharp << ksp << " " << calculateStressSharpKinkPair(0.5 * b, ksp) << endl;
	}
